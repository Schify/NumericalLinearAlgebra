function [reg_min,G,reg_param] = gcv_mod(U,s,b,reg_pars,method)
% Modified version of the function for homework
...
% Adapted for NLA Homework by Andras Schifferer in Dec 2023
...
npoints = length(reg_pars);                      % Number of points on the curve.
...
%   for i=npoints-1:-1:1, reg_param(i) = ratio*reg_param(i+1); end
  ...
  if (find_min)
    ...
    reg_min = reg_param(minGi); % Minimizer.
