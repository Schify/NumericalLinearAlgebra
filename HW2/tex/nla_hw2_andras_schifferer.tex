...
%		Since by definition $\textbf{r}_0^{LS} \in \mathcal{K}_i \left(A^T A,\textbf{r}_0^{LS} \right)$, we can write it using the orthogonal basis as $\textbf{r}_0^{LS} = Q_k \textbf{z}$.
	...
	\begin{figure}
		...
		\begin{tikzpicture}[scale=0.7]
			...
			\draw[magenta, dashed, thick, variable=\t, domain=-3.5:6.12,,postaction={decorate,decoration={markings,mark=between positions 0.1 and 0.8 step 10mm with {\arrow[magenta]{stealth};}}},
				]
				 plot ({3.1+ln(1+exp(\t)/2)}, {4.2+ln(1+exp(-\t)-12*exp(\t-8.7))}) ;
