...
%		Since by definition $\textbf{r}_0^{LS} \in \mathcal{K}_i \left(A^T A,\textbf{r}_0^{LS} \right)$, we can write it using the orthogonal basis as $\textbf{r}_0^{LS} = Q_k \textbf{z}$.
	...
	\begin{figure}
		...
		\begin{tikzpicture}[scale=0.7]
			...
			}] ({2.0*\wth}, 4) -| (8, {2.5*\wth}) node[pos=0.15, orange, above left] {$\lambda$ not large} node[pos=0.9, orange, above, rotate=-90] {$\lambda$ small};
			\draw[] (-\wth, 4) -- (\wth, 4) node[pos=0, left] {$\log \left\|\bar{\textbf{x}}\right\|$};
			\draw[] (8,-\wth) -- (8,\wth) node[pos=0, below] {$\log \left\|\bar{\textbf{b}}\right\|$};
			
			\draw[magenta, dashed, thick, variable=\t, domain=-3.5:6,,postaction={decorate,decoration={markings,mark=between positions 0.1 and 0.8 step 10mm with {\arrow[magenta]{stealth};}}},
				]
				 plot ({3.1+ln(1+exp(\t)/2)}, {4.2+ln(1+exp(-\t)-2*exp(\t-7))}) ;
			\node[draw, circle, fill, cyan, inner sep = 1.5pt] (Lopt) at ({3.1+ln(1+exp(0)/2)}, {4.2+ln(1+exp(-0)-2*exp(0-7))}){};
			\draw[<-, cyan] (Lopt) -- +(2,1.5) node[pos=1, above right] {ideal $\lambda$};
		\end{tikzpicture}
		\caption{L-curve theory. $m $ is the number of nonzero singular values of the matrix $A$. The errors associated to $\textbf{e}$ are in {\color{red}red}, while those linked to $\bar{\textbf{b}}$ are in {\color{green}green}. The overall behaviour belonging to $\bar{\textbf{x}}_\lambda$ is shown in {\color{magenta}magenta} }\label{fig:Lcurve-theory}
	\end{figure}
	\newpage
